...
# ВОТ ОНО! Настройка локали для корректной работы чисел в MPV (критично для Android/Linux)
try:
...
class IPTVWorker(QThread):
    ...
    def run(self):
        try:
            ...
            self.finished.emit(channels, epg_db, f"Успех: {len(channels)} каналов")
