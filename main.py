...
# Настройка локали для корректной работы чисел в MPV
try:
...
class IPTVWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # 1. ПОЛНЫЙ ПАРСИНГ EPG (XMLTV .gz)
            if self.epg_url:
                epg_db = self._parse_xmltv_pro(self.epg_url, headers)

            # 2. ПРОФЕССИОНАЛЬНЫЙ ПАРСИНГ ПРОТОКОЛОВ
            if self.proto == "M3U":
                channels = self._parse_m3u_pro(headers)
            elif self.proto == "XTREAM":
                channels = self._parse_xtream_pro(headers)
            elif self.proto == "STALKER":
                channels = self._parse_stalker_pro(headers)
            
            self.finished.emit(channels, epg_db, f"Найдено: {len(channels)}")
        except Exception as e:
            self.error.emit(str(e))

    def _parse_xmltv_pro(self, url, h):
        ...
        for p in tree.findall('programme'):
            ...
            epg[cid].append({
                ...
                "start": start,
                "time": f"{dt_s.strftime('%H:%M')} - {dt_e.strftime('%H:%M')}"
            })
        return epg

    def _parse_m3u_pro(self, h):
        r = requests.get(self.host, headers=h, timeout=20)
        r.raise_for_status()
        pattern = re.compile(r'#EXTINF:.*?(?:tvg-id="(.*?)")?.*?(?:tvg-logo="(.*?)")?.*?(?:catchup="(.*?)")?.*?,(.*?)\n(http.*?)(?:\n|$)', re.M)
        return [{"id": m.group(1) or m.group(4).strip(), "logo": m.group(2) or "", "catchup": m.group(3) or "default", "name": m.group(4).strip(), "url": m.group(5).strip()} for m in pattern.finditer(r.text)]

    def _parse_xtream_pro(self, h):
    ...
    def _parse_stalker_pro(self, h):
        ...
        res = requests.get(api, headers=h, timeout=15).json()
        return [{"id": str(c.get('tvg_id', c.get('name'))), "name": c['name'], "logo": "", "url": c['url'].split(' ')[-1], "catchup": "none"} for c in res['js']]
...
class EPGModel(QAbstractListModel):
    ...
    def data(self, index, role):
        ...
        # Роли: 201-Title, 202-Time, 203-Desc, 204-StartRaw
    ...
    def _init_db(self):
        ...
        if res:
            try:
                ...
                self._status = f"Vault: {len(self._channels)} channels"
            except Exception: pass
